...
@tool
def ask_human_1(message: str) -> str: 
    """
    Ask human 1 for help or feedback.
    Args:
        message: The message to send to the human
    """
    try:
        message = f"For Human 1: {message}"
        result = subprocess.run(["send_slack_message", "--message", message], 
                              capture_output=True, text=True)
        return f"Human: {result.stdout}"
    except Exception as e:
...
@tool
def ask_human_2(message: str) -> str:
    """
    Ask human 2 for help or feedback.
    Args:
        message: The message to send to the human
    """
    try:
        message = f"For Human 2: {message}"
        result = subprocess.run(["send_slack_message", "--message", message], 
                              capture_output=True, text=True)
        return f"Human: {result.stdout}"
    except Exception as e:
...
agent = ToolCallingAgent(tools=[execute_bash, edit_file, DuckDuckGoSearchTool(), explore_repo, analyze_code, check_mail, send_message, write_to_blog, read_blog, ask_human_1, ask_human_2], model=model, max_steps=999, remove_final_answer_tool=True, planning_interval=3, stream_json_logs=True, json_logs_path=f"/home/{os.getenv('AGENT_NAME')}/logs/logs.json")
