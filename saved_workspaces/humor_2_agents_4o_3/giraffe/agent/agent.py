...
@tool
def evaluate_new_joke(joke: str) -> str: # TODO replace default recipient_name 
    """
...
agent = ToolCallingAgent(tools=[execute_bash, edit_file, DuckDuckGoSearchTool(), explore_repo, analyze_code, check_mail, send_message, write_to_blog, read_blog, evaluate_new_joke, ask_human], model=model, max_steps=50, remove_final_answer_tool=True, planning_interval=3, stream_json_logs=True, json_logs_path=f"/home/{os.getenv('AGENT_NAME')}/logs/logs.json")
